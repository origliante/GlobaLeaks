...
class GLBGPG(object):
    ...
    def __init__(self, receiver_desc):
        ...
        if 'gpg_key_status' in receiver_desc and \
            ...
            log.err("Requested GPG initialization for a receiver without GPG configured! %s" % receiver_desc['username'])
    ...
    def validate_key(self, armored_key):
        ...
        try:
            key, others = pgpy.PGPKey.from_blob(sanitized_gpgasc)
            skey = str(key)
            key_len = len(skey)
        ...
        self.keyring[ fingerprint ] = (key, others,)

        key, others = self.keyring[ fingerprint ]
        keyinfo = u"Key length %s" % key_len
        try:
    ...
    def encrypt_file(self, plainpath, filestream, output_path):
        ...
        @return:
        """
        key, others = self.keyring[ self.receiver_desc['gpg_key_fingerprint'] ]
        #try:
        msg = pgpy.PGPMessage.new( filestream.read() )
        enc_msg = key.encrypt(msg)
        enc_msg_data = str( enc_msg )
        enc_msg_len = len( enc_msg_data )

        #except:
        #    log.err("Failure in encrypting file %s %s (%s)" %
        #            (   plainpath,
        ...
        try:
            with open(encrypted_path, "w+") as f:
                f.write( enc_msg_data )
        except Exception as excep:
            log.err("Error in writing GPG file output: %s (%s) bytes %d" %
                    (excep.message, encrypted_path, enc_msg_len ))
            raise errors.InternalServerError("Error in writing [%s]" % excep.message)

        return encrypted_path, enc_msg_len


    #TODO: reuse in encrypt_file?
    def encrypt_message(self, plaintext):
        ...
        enc_msg_len = len( enc_msg_data )
        #except:
        #    log.err("Failure in encrypting %d bytes for %s (%s)" % (len(plaintext),
        #                                                           self.receiver_desc['username'],
        #                                                           self.receiver_desc['gpg_key_fingerprint']))
        #    raise errors.GPGKeyInvalid

        log.debug("Encrypted for %s (%s) %d byte of plain data (%d cipher output)" %
                  ( self.receiver_desc['username'],
                    self.receiver_desc['gpg_key_fingerprint'],
                    len(plaintext),
                    enc_msg_len) )
...
def get_expirations(keylist):
    ...
    expirations = {}
    for key in keylist:
        ak, others = pgpy.PGPKey.from_blob( key )
        expirations[ ak.fingerprint ] = ak.created.strftime('%s')
    return expirations
